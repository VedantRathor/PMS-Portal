...
function LogIndividual() {
  ...
  useEffect(() => {
    ...
    getProjectLogData()
  }, [selectProjectData,selectStatus,selectSort])
              ...
              logData.map((eachproject) => (
                eachproject.tasks.map((eachTask) => (
                  eachTask.logs.map((eachLog) => (
                    <AllLogs generatePDF={generatePDF} eachproject={eachproject} eachTask={eachTask} eachLog={eachLog} parentFunctionDisplayData={parentFunctionDisplayData} />
                  ))
                ))
              ))
              : <>No Logs Available</>}
